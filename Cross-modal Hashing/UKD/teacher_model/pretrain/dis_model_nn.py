...
class DIS():
	def __init__(self, image_dim, text_dim, hidden_dim, output_dim, weight_decay, learning_rate, beta, gamma, loss = 'svm', param = None):
		...
		self.image_hash = tf.cast(self.image_sig + 0.5, tf.int32)  # 通过强制类型转换四舍五入
		...
		self.pred_distance = tf.reduce_sum(tf.square(self.image_sig - self.text_sig), 1)  # 每行加起来 n×1
		self.hash_score = tf.reduce_sum(tf.cast(tf.equal(self.image_hash, self.text_hash), tf.float32), 1) # 每行加起来 n×1
			...
			with tf.name_scope('log_loss'):
				...
				self.i2t_loss = -tf.reduce_mean(tf.log(tf.sigmoid(self.pred_i2t_neg_distance - self.pred_distance))) + \
								self.weight_decay * (tf.nn.l2_loss(self.Wq_1) + tf.nn.l2_loss(self.Wq_2)
												   ...
												   + tf.nn.l2_loss(self.Wc_1) + tf.nn.l2_loss(self.Wc_2)
												   + tf.nn.l2_loss(self.Bc_1) + tf.nn.l2_loss(self.Bc_2))
				self.i2t_reward = tf.reshape(tf.log(tf.sigmoid(self.pred_i2t_neg_distance - self.pred_distance)), [-1])  # n×1展开成一维的
				...
				self.t2i_loss = -tf.reduce_mean(tf.log(tf.sigmoid(self.pred_t2i_neg_distance - self.pred_distance))) + \
								self.weight_decay * (tf.nn.l2_loss(self.Wq_1) + tf.nn.l2_loss(self.Wq_2)
												   ...
												   + tf.nn.l2_loss(self.Wc_1) + tf.nn.l2_loss(self.Wc_2)
												   + tf.nn.l2_loss(self.Bc_1) + tf.nn.l2_loss(self.Bc_2))
				self.t2i_reward = tf.reshape(tf.log(tf.sigmoid(self.pred_t2i_neg_distance - self.pred_distance)), [-1])  # n×1展开成一维的
