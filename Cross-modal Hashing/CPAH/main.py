...
from utils import calc_map_k, pr_curve, p_top_k, write_pickle, pr_curve3, p_top_k2, calc_map_k2, \
    calc_map_rad2
...
from datasets.dset import MY_DATASET
...
def train(**kwargs):
    ...
    # test
    i_query_data = MY_DATASET(opt=opt, train=False, database=False).images
    i_db_data = MY_DATASET(opt=opt, train=False, database=True).images
    t_query_data = MY_DATASET(opt=opt, train=False, database=False).txt
    t_db_data = MY_DATASET(opt=opt, train=False, database=True).txt
    ...
    query_labels = MY_DATASET(opt=opt, train=False, database=False).labels
    db_labels = MY_DATASET(opt=opt, train=False, database=True).labels
    query_labels = query_labels.to(opt.device)
    db_labels = db_labels.to(opt.device)

    opt.query_size = query_labels.shape[0]
    opt.db_size = db_labels.shape[0]
    ...
    for epoch in range(opt.max_epoch):
        ...
        e_losses = {'adv': 0, 'class': 0, 'quant': 0, 'pairwise': 0}
        # for i, (ind, img, txt, label) in tqdm(enumerate(train_dataloader)):
        for i, (img, txt, label, ind) in enumerate(train_dataloader):
            # print(i)
            imgs = img.to(opt.device)
            txt = txt.to(opt.device)
            labels = label.to(opt.device)
