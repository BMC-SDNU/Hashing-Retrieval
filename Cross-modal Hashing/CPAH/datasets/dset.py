...
class MY_DATASET(torch.utils.data.Dataset):
    def __init__(self, opt, train=True, database=False):
        ...
        if train:
            ...
            self.images = torch.as_tensor(all_data['I_tr'][:].T, dtype=torch.float32)
        elif database:
            # retrieval
            ...
            self.images = torch.as_tensor(all_data['I_db'][:].T, dtype=torch.float32)
        else:
            # test
            ...
            self.images = torch.as_tensor(all_data['I_te'][:].T, dtype=torch.float32)
